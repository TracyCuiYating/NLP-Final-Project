"""
Final project created by Tracy Cui and Tina Tao
The model building part is cited from hmm_model_build.py created by Tracy Cui and Elen Liu
The Viterbi algorithm is cited from hmm_tagger.py created by Tracy Cui and Elen Liu
"""
import numpy as np
import re
...
def correction(sentence):
    ...
    return max_prob_sent,corrected_words,wordcount
...
def correction_text(text):
    ...
    for i in range(len(text)):
        sentence,wordlist,wordcount = correction(text[i])
        tot_wordcount += 1
    ...
    return correct_words,corrected_text,tot_wordcount
...
def main():
    ...
    #print("The sentences we want to correct are", text)
    corrected_words, corrected_text,cor_word_count = correction_text(text[:25])
    acc_rate = accuracy(corrected_words, errors,cor_word_count)
    #print("The corrected sentence are",corrected_text)
    print("Our success rate is", acc_rate)
